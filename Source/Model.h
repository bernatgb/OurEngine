...
public:
	...
	float4 GetMaxPoint() const {
		return m_ModelMatrix * float4(m_Max.x, m_Max.y, m_Max.z, 1.0f);
	};
	float4 GetMinPoint() const {
		return m_ModelMatrix * float4(m_Min.x, m_Min.y, m_Min.z, 1.0f);
	};
	float4 GetCenter() const {
		return m_ModelMatrix * float4(m_Min.x + (m_Max.x-m_Min.x)/2.0f, m_Min.y + (m_Max.y-m_Min.y)/2.0f, m_Min.z + (m_Max.z-m_Min.z)/2.0f, 1.0f);
	};
	float4 GetBase() const {
		return m_ModelMatrix * float4(m_Min.x + (m_Max.x-m_Min.x)/2.0f, m_Min.y, m_Min.z + (m_Max.z-m_Min.z)/2.0f, 1.0f);
	};
